...
class PreferredAreasTableSeeder extends Seeder {

    public function run()
    {
        $seoul_id = PreferredAreaGroup::where('name', '서울특별시')->first()->id;
        ...
        $gyung_gi_do_id = PreferredAreaGroup::where('name', '경기도')->first()->id;
        ...
        foreach($seoul_borough_array as $borough) {
            PreferredArea::create([
                'preferred_area_group_id' => $seoul_id,
                'name' => $borough
            ]);
        }
        ...
        foreach($gyung_gi_do_city_array as $city) {
            PreferredArea::create([
                'preferred_area_group_id' => $gyung_gi_do_id,
                'name' => $city
            ]);
        }
    }
